    ...
    <section class="breadcrumbs-custom">
        <div class="parallax-container" data-parallax-img="{{ asset('images/bg-profile.jpg') }}">
            <div class="breadcrumbs-custom-body parallax-content context-dark">
                <div class="container">
                    <h2 class="text-transform-capitalize breadcrumbs-custom-title"> {{ucfirst($reservations[0]->status)}} reservations</h2>
            ...
            <table class="table table-responsive-md">
                ...
                @foreach($reservations as $reservation)
                    <tr>
                        ...
                        <td><img width="90" height="90" src="{{asset('/images/uploads/packs/'.$reservation->picture)}}"/><p>{{$reservation->title}}</p></td>
                        ...
                        <td>
                            <form action="{{route('reservation.changeStatus',$reservation->id)}}" method="post">
                                @csrf
                                <button name="status" value="{{$reservation->status}}" class="btn btn-primary">Claimed</button>
                            </form>
                        </td>
